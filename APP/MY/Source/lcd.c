...
u16 BACK_COLOR;   //背景色
void Brightness_set(u16 brightness)
{
  TIM_SetCompare4(TIM3,brightness*10);
}
/******************************************************************************
      函数说明：LCD串行数据写入函数
      入口数据：dat  要写入的串行数据
...
void Lcd_Init(void)
{
  ...
  GPIO_Init(GPIOB, &GPIO_InitStructure);//初始化
	GPIO_SetBits(GPIOB,GPIO_Pin_0|GPIO_Pin_2|GPIO_Pin_10);
...
void LCD_ShowPicture(u16 x1,u16 y1,u16 x2,u16 y2)
{
	  ...
	  LCD_Address_Set(x1,y1,x2,y2);
		for(i=0;i<135*240;i++)
	  { 	
			 LCD_WR_DATA8(gImage_1[i*2+1]);
		 	LCD_WR_DATA8(gImage_1[i*2]);
	  }			
}

//void LCD_ShowPicture1(u16 x1,u16 y1,u16 x2,u16 y2,u8 *p,u32 pian)
//{
//	  u32 i;
//	  LCD_Address_Set(x1,y1,x2,y2);
//		for(i=0;i<135*240;i++)
//	  { 	
//			LCD_WR_DATA8(p[pian+i*2+1]);
//			LCD_WR_DATA8(p[pian+i*2]);
//	  }			
//}
